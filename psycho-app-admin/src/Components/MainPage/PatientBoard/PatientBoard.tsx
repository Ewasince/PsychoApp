...
export const PatientBoard = () => {
  ...
  const [storiesByWeek, setStoriesByWeek] = useState<Map<number, IStoryDto[]>>(
    new Map([]),
  );
  const [moodsByWeek, setMoodsByWeek] = useState<Map<number, MoodsArray>>(
    new Map([]),
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const storiesMinDateData = await getPatientStoriesMinDate(
          patientId as string,
        );
        const minDateStories = dayjs.unix(storiesMinDateData.data.minDate);
        const todayDateStories = dayjs();
        const storiesData = await getPatientStories(patientId as string, {
        ...
        const moodsMinDateData = await getPatientMoodsMinDate(
          patientId as string,
        );
        const minDateMoods = dayjs.unix(moodsMinDateData.data.minDate);
        const todayDateMoods = dayjs();
        const moodsData = await getPatientMoods(patientId as string, {
  ...
  function processStoriesByWeek(stories: IStory[]): number {
    ...
    for (const story of stories) {
      ...
      const storyDto: IStoryDto = {
        // TODO: конечно хуёвый способ так делать, нужно это в отдельный класс вынести
        id: story.id,
  ...
  const CustomMenu = () => (
    <>
      ...
      >
        <MenuItem id="delete-patient" onClick={handleCloseMenu}>
          Удалить пациента
        </MenuItem>
      </Menu>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Вы уверены, что хотите удалить <b>{patient?.firstName}?</b> Это
            действие нельзя отменить
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button id="delete-cancel" onClick={handleCloseAlert}>
            Отмена
          </Button>
  ...
  return (
    <div className={`flex h-full flex-col`}>
      ...
      />
      <div
        className={`flex flex-grow flex-col items-center space-y-5 overflow-hidden`}
      >
        ...
        <div className="flex w-full flex-grow flex-col justify-between space-y-3 overflow-hidden">
          <div className={`h-2/3`}>
            <KptTable weekIndex={currentPage} storiesByWeek={storiesByWeek} />
          </div>
          <div className={`h-1/3 overflow-hidden`}>
            <MoodGraph weekIndex={currentPage} moodsByWeek={moodsByWeek} />
          </div>
        </div>
      </div>
    </div>
...
function getWeekNumFromDate(date: Dayjs): number {
  // 0 week means is current, 1 – week ago
